...
def main():
    parser = argparse.ArgumentParser(description="Convert Vaultwarden export JSON to KeePass database.")
    parser.add_argument("-v", "--verbose", action="count", default=0, help="Logging verbosity level")
    parser.add_argument("--keepass-db", required=True, help="Path to the KeePass database file to create")
    parser.add_argument("--keepass-password", required=True, help="Master password for the KeePass database")
    parser.add_argument("--vaultwarden-json", required=True, help="Path to the Vaultwarden JSON export file")
